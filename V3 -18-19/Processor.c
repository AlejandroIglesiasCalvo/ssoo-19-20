...
void Processor_DecodeAndExecuteInstruction()
{
	...
	case 'o': // Make a operating system routine in entry point indicated by operand1
		...
		if (Processor_PSW_BitState(EXECUTION_MODE_BIT))
		{
			...
			ComputerSystem_DebugMessage(130, HARDWARE, operationCode,
										operand1, operand2, PID, registerPC_CPU, registerAccumulator_CPU,
	...
	ComputerSystem_DebugMessage(
		130, HARDWARE, operationCode, operand1, operand2, PID, registerPC_CPU, registerAccumulator_CPU,
		registerPSW_CPU, Processor_ShowPSW());
}
