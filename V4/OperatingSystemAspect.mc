...
void OperatingSystem_HandleException()
{
 ...
 {
 case DIVISIONBYZERO:
  ComputerSystem_DebugMessage(140, 'p', executingProcessID, programList[processTable[executingProcessID].programListIndex]->executableName, "division by zero");
  break;
 case INVALIDPROCESSORMODE:
  ComputerSystem_DebugMessage(140, 'p', executingProcessID, programList[processTable[executingProcessID].programListIndex]->executableName, "invalid processor mode");
  break;
 case INVALIDADDRESS:
  ComputerSystem_DebugMessage(140, 'p', executingProcessID, programList[processTable[executingProcessID].programListIndex]->executableName, "invalid address");
  break;
...
void OperatingSystem_HandleSystemCall()
{
 ...
 case SYSCALL_END:
  ...
  break;
 case SYSCALL_YIELD:
  PID_para_Procesador = executingProcessID;
...
void Change_State(PID1, antiguo, nuevo)
{
 ...
 {
  OperatingSystem_ShowTime('p');
  ComputerSystem_DebugMessage(111, 'p', PID1, programList[processTable[PID1].programListIndex]->executableName, statesNames[antiguo]);
 }
 else
 {
  OperatingSystem_ShowTime('p');
