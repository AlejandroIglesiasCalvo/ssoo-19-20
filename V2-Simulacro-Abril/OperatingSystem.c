...
void OperatingSystem_PCBInitialization(int PID, int initialPhysicalAddress, int processSize, int priority, int processPLIndex)
{
	...
	//Simualcro abril
	processTable[PID].BanderaSimulacro = 0;
...
void OperatingSystem_HandleSystemCall()
{
	...
	case SYSCALL_YIELD: //v1 E12
		if (processTable[executingProcessID].BanderaSimulacro == 0)
		{
			processTable[executingProcessID].BanderaSimulacro = 1;
			la_Magia_Del_Yield(executingProcessID);
		}
		else
		{
			ComputerSystem_DebugMessage(666, SHORTTERMSCHEDULE, executingProcessID, programList[processTable[executingProcessID].programListIndex]->executableName);
		}
		break;
...
int EOEOEOEOEORROOOOOR()
{
	if (processTable[executingProcessID].BanderaSimulacro == 0)
	{
		processTable[executingProcessID].BanderaSimulacro = 1;
	}
	return processTable[executingProcessID].BanderaSimulacro;
}