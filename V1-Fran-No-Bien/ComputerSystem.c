...
void ComputerSystem_PrintProgramList(){
	...
	int a;
	for(a = 1; programList[a]!=NULL && a < PROGRAMSMAXNUMBER ; a++){
		if(programList[a]->type==USERPROGRAM){
			ComputerSystem_DebugMessage(102, INIT, programList[a]->executableName, programList[a]->arrivalTime);
		}
	}
}